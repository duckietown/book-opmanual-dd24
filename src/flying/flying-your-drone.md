...
Motor efficiency, prop weight, and battery charge all affect it, so the default midpoint (`1600`)
is set intentionally high. Without tuning it to your build, the drone will shoot upward faster
...
*   Safety goggles
---
*   A throttle midpoint calibrated to your drone so it hovers instead of climbing
```
...
7.  Restart the nodes and fly. The drone should lift off and settle at roughly 20 cm. Some
    oscillation on the way up is normal.
